...
moonbit-pixelkit 是一个面向 2D 像素游戏、战棋原型、迷宫/Roguelike Demo 和算法教学场景的 MoonBit 工具库。项目提供轻量的 tile map 数据结构、ASCII/CSV 地图解析、可通行性查询、移动代价、BFS 可达区域搜索与累计代价、A* 路径搜索、高层移动预览、路径合法性校验、路径代价统计和调试渲染能力。
...
- 基于优先队列 Dijkstra 的可达区域搜索，用于固定移动预算内的角色行动范围预览，并返回每个可达格子的最小累计移动代价。
- 高层 `MovementPreview` API，一次性返回可达范围、目标是否可达、目标累计代价和目标路径，减少游戏原型中的胶水代码。
- `TileMap::validate_path`，用于检查外部输入路径是否逐步合法、是否越界或撞墙，并返回步数和总代价。
- 斜向移动默认禁止穿过相邻墙体的夹角，可按场景显式开启切角行为，保证寻路、可达范围和路径校验遵循同一套规则。
...
- Mooncakes 0.2.0 发布页：https://mooncakes.io/docs/ttxiangshang/moonbit-pixelkit
...
- 持续维护 Mooncakes 已发布的 0.2.0 版本，并在后续功能稳定后发布下一版。
- 扩展结构化错误类型，为更多编辑器与运行时诊断场景提供稳定错误码。
- 继续补充面向编辑器/关卡工具的地图预览和调试辅助能力。
- 在真实需求驱动下扩展 Tiled JSON 的 chunk 和压缩图层支持。
